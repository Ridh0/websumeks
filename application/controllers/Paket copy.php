...
class Paket extends CI_Controller
{
	...
	function index()
	{
		$jumlah = $this->model_app->view('tb_paket')->num_rows();
		...
		if (is_numeric($dari)) {
			...
			if ($this->input->post('cari') != '') {
				...
				$data['record'] = $this->model_app->cari_paket(filter($this->input->post('cari')));
			} else {
				$data['title'] = 'Produk' . ' - ' . title();
				$data['breadcrumb'] = 'Produk';
				$data['judul'] = 'Semua Produk';
				$data['record'] = $this->model_app->view_ordering_limit('tb_paket', 'id_paket', 'DESC', $dari, $config['per_page']);
				$this->pagination->initialize($config);
			}
			$data['artikel'] = $this->model_artikel->semua_artikel(0, 15);
			$this->template->load('home/template', 'home/paket/view_paket', $data);
	...
	function kategori()
	{
		$cek = $this->model_app->edit('tb_paket_kategoriproduk', array('kategori_seo' => $this->uri->segment(3)))->row_array();
		$jumlah = $this->model_app->view_where('tb_paket', array('id_kategori_paket' => $cek['id_kategori_paket']))->num_rows();
		...
		if (is_numeric($dari)) {
			...
			$data['record'] = $this->model_app->view_where_ordering_limit('tb_paket', array('id_kategori_paket' => $cek['id_kategori_paket']), 'id_paket', 'DESC', $dari, $config['per_page']);
			$this->pagination->initialize($config);
			$this->template->load('home/template', 'home/paket/view_paket', $data);
		} else {
			redirect('main');
	...
	function detail()
	{
		$query = $this->model_app->edit('tb_paket', array('username_paket' => $this->uri->segment(3)));
		if ($query->num_rows() >= 1) {
			$cek = $query->row_array();
			$data['title'] = "$cek[nama_paket]";
			$data['breadcrumb'] = "$cek[nama_paket]";
			$data['judul'] = "$cek[nama_paket]";
			$data['row'] = $this->model_app->view_where('tb_paket', array('id_paket' => $cek['id_paket']))->row_array();

			$this->template->load('home/template', 'home/paket/view_detail_paket', $data);
		} else {
			redirect('main');
		}
	}
	...
	function cart()
	{
        ...
        if ($id_paket != '') {
            ...
            if ($cek >= 1) {
                $this->db->query("UPDATE tb_paket_penjualantemp SET jumlah=jumlah+$jumlah where session='" . $this->session->idp . "' AND id_paket='$id_paket'");
            } else {
	...
	function kurirdata()
	{
		...
		if (!empty($d['rajaongkir']['results'])) {
			...
			$this->load->view('home/paket/kurirdata', $data);
		} else {
			$data['ongkir'] = 0;
			$this->load->view('home/paket/kurirdata', $data);
		}
	...
	function checkouts()
	{
		if (isset($_POST['submit'])) {
			if ($this->session->idp != '') {
				...
				$cart = $this->model_app->view_where('tb_paket_penjualantemp', array('session' => $this->session->idp));
				...
				//$kons = $this->model_app->view_join_where_one('tb_paket', 'tb_kota', 'kota_id', array('id_pengguna' => $this->session->id_pengguna))->row_array();
				...
				$this->template->load('home/template', 'home/paket/view_transaksi_berhasil', $data);
			} else {
				redirect('paket/cart');
			}
		} else {
			...
			if ($this->session->id_pengguna) {
				$cek = $this->model_app->view_where('tb_paket_penjualantemp', array('session' => $this->session->idp));
					...
					//$data['record'] = $this->model_app->view_join_rows('tb_paket_penjualantemp', 'tb_paket', 'id_paket', array('session' => $this->session->idp), 'id_penjualan_detail', 'ASC');
					$this->template->load('home/template', 'home/paket/view_checkouts', $data);
	...
	function print_invoice()
	{
		...
		$data['rows'] = $this->db->query("SELECT * FROM tb_paket_penjualan a JOIN tb_kota b ON a.p_kota=b.kota_id where a.kode_transaksi='$idp'")->row_array();
		$data['record'] = $this->db->query("SELECT a.kode_transaksi, a.waktu_transaksi, b.*, c.nama_paket, c.satuan, c.berat, c.diskon FROM `tb_paket_penjualan` a JOIN tb_paket_penjualandetail b ON a.id_penjualan=b.id_penjualan JOIN tb_paket c ON b.id_paket=c.id_paket where a.kode_transaksi='" . $this->uri->segment(3) . "'");
		//$this->load->view('home/paket/print_invoice', $data);
		$this->load->library('pdf');
		$this->pdf->setPaper('A4', 'potrait');
		$this->pdf->filename = "Invoince-" . $idp . ".pdf";
		$this->pdf->load_view('home/paket/print_invoice', $data);
	}
	...
	function review()
	{
		if (isset($_POST['submit'])) {
			...
			$cek = $this->db->query("SELECT * FROM `tb_ulasan` where id_pembeli='$pembeli' AND id_paket='$produk'");
			if ($cek->num_rows() >= 1) {

				$this->db->query("UPDATE tb_ulasan SET bintang='$bintang',ulasan='$ulasan' where id_pembeli='$pembeli' AND id_paket='$produk'");
				redirect("paket/detail/$seo");
			} else {
				$data = array(
					'id_paket' => $produk,
					'id_pembeli' => $pembeli,
