    ...
    function index()
    {
        ...
        if ($id_paket != '') {
            if ($stok < $this->input->post('jumlah') or $stok <= '0') {
                $produk = $this->model_app->edit('tb_paket', array('id_paket' => $id_paket))->row_array();
                $produk_cek = filter($produk['nama_paket']);
        ...
        if (isset($_POST['submit'])) {
            if ($this->session->idp != '') {
                ...
                foreach ($belanjaan as $row) {
                    ...
                    $diskon_total = 0 * $row['jumlah'];

                    $message .= "<tr>
									<td>$no</td>
				                    <td>$row[nama_paket]</td>
				                    <td>" . rupiah($row['harga_paket'] - 0) . " $diskon</td>
				                    <td>$row[jumlah]</td>
