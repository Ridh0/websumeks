...
class Paket extends CI_Controller
{
	...
	function kategori()
	{
		...
		if (is_numeric($dari)) {
			...
			$data['record'] = $this->model_app->view_where_ordering_limit('tb_toko_produk', array('id_kategori_produk' => $cek['id_kategori_produk']), 'id_paket', 'DESC', $dari, $config['per_page']);
			$this->pagination->initialize($config);
	...
	function keranjang()
	{
		$id_paket   = filter($this->input->post('id_paket'));
		...
		if ($id_paket != '') {
			if ($stok < $this->input->post('jumlah') or $stok <= '0') {
				$produk = $this->model_app->edit('tb_toko_produk', array('id_paket' => $id_paket))->row_array();
				$produk_cek = filter($produk['nama_produk']);
				echo "<script>window.alert('Maaf, Stok untuk pemesanan Produk - $produk_cek Tidak Mencukupi!');
                                  window.location=('" . base_url() . "produk/detail/$produk[username_paket]')</script>";
			} else {
				...
				if ($cek >= 1) {
					...
					$harga = $this->model_app->view_where('tb_toko_produk', array('id_paket' => $id_paket))->row_array();
					$data = array(
			...
			$data['record'] = $this->model_app->view_join_rows('tb_paket_penjualantemp', 'tb_toko_produk', 'id_paket', array('session' => $this->session->idp), 'id_penjualan_detail', 'ASC');
		...
		if (isset($_POST['submit'])) {
			if ($this->session->idp != '') {
				...
				foreach ($keranjang->result_array() as $row) {
					...
					$q = $this->db->get_where('tb_toko_produk', array('id_paket' => $row['id_paket']));
				...
				$no = 1;
				$belanjaan = $this->model_app->view_join_where('tb_paket_penjualandetail', 'tb_toko_produk', 'id_paket', array('id_penjualan' => $idp), 'id_penjualan_detail', 'ASC');
			...
			if ($this->session->id_pengguna) {
				...
				if ($cek->num_rows() >= 1) {
					...
					$this->db->join('tb_toko_produk', 'tb_paket_penjualantemp.id_paket=tb_toko_produk.id_paket');
	...
	function print_invoice()
	{
		...
		$data['record'] = $this->db->query("SELECT a.kode_transaksi, a.waktu_transaksi, b.*, c.nama_paket FROM `tb_paket_penjualan` a JOIN tb_paket_penjualandetail b ON a.id_penjualan=b.id_penjualan JOIN tb_paket c ON b.id_paket=c.id_paket where a.kode_transaksi='" . $this->uri->segment(3) . "'");
