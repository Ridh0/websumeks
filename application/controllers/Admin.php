...
class Admin extends CI_Controller
{
	...
	function paket()
	{
		$data['title'] = 'Paket - Zamanet Store';
		$data['record'] = $this->model_app->view_ordering('tb_paket', 'nama_paket', 'ACS');
		$this->template->load('admin/template', 'admin/paket/view_paket', $data);
	}
		...
		if (isset($_POST['submit'])) {
			...
			if ($hasil['file_name'] == '') {
				$data = array(
					...
					'nama_paket' => $this->input->post('b'),
					'username_paket' => $this->db->escape_str(seo_title($this->input->post('b'))),
					
					'harga_paket' => $this->input->post('d'),
			...
			$this->model_app->insert('tb_paket', $data);
			redirect('admin/paket');
		} else {

			$data['title'] = 'Tambah paket - Zamanet Store';
			$this->template->load('admin/template', 'admin/paket/view_paket_tambah', $data);
		}
	}
	...
	function edit_paket()
	{
		...
		if (isset($_POST['submit'])) {
			...
			if ($hasil['file_name'] == '') {
				...
				$query = $this->db->get_where('tb_paket', array('id_paket' => $this->input->post('id')));
				$row = $query->row();
			
			}

			$where = array('id_paket' => $this->input->post('id'));
			$this->model_app->update('tb_paket', $data, $where);
			redirect('admin/paket');
		} else {

			$data['title'] = 'Edit - Zamanet Store';
			$data['rows'] = $this->model_app->edit('tb_paket', array('id_paket' => $id))->row_array();
			$this->template->load('admin/template', 'admin/paket/view_paket_edit', $data);
		}
	}
	...
	function delete_paket($id)
	{
		...
		$this->model_app->delete('tb_paket', $where);
		echo json_encode(array("status" => TRUE));
	}
