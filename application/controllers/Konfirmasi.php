...
class Konfirmasi extends CI_Controller
{
	...
	function index()
	{
		if (isset($_GET['q'])) {
			...
			$cek = $this->model_app->view_where('tb_paket_penjualan', array('kode_transaksi' => $kode_transaksi));
			$row = $this->db->query("SELECT id_penjualan FROM `tb_paket_penjualan` where kode_transaksi='$kode_transaksi'")->row_array();
			$data['record'] = $this->model_app->view('tb_toko_rekening');

			$data['total'] = $this->db->query("SELECT a.kode_transaksi,  a.proses, sum((b.harga_paket*b.jumlah)-(0*b.jumlah)) as total, sum(0*b.jumlah) as total_berat FROM `tb_paket_penjualan` a JOIN tb_paket_penjualandetail b ON a.id_penjualan=b.id_penjualan JOIN tb_paket c ON b.id_paket=c.id_paket where a.kode_transaksi='" . $kode_transaksi . "'")->row_array();
			$data['rows'] = $this->model_app->view_where('tb_paket_penjualan', array('id_penjualan' => $row['id_penjualan']))->row_array();
			...
			$this->template->load('home/template', 'home/paket/view_konfirmasi_pembayaran', $data);
		} else {
			if (isset($_POST['submit1'])) {
				...
				$data['title'] = 'Konfirmasi Pembayaran';
				$data['breadcrumb'] = 'Konfirmasi Pembayaran';
				$this->template->load('home/template', 'home/paket/view_konfirmasi', $data);
			}
		}
	}
	...
	function form()
	{
		...
		if (!$this->upload->do_upload('userfile')) {
			...
			$this->model_app->update('tb_paket_penjualan', $data1, $where);
	...
	function tracking()
	{
		if (isset($_POST['submit1']) or $this->uri->segment(3) != '') {
			...
			if ($cek->num_rows() >= 1) {
				...
				$data['rows'] = $this->db->query("SELECT * FROM tb_paket_penjualan a JOIN tb_pengguna b ON a.id_pembeli=b.id_pengguna JOIN tb_alamat c ON b.id_alamat=c.id_alamat JOIN tb_kota d ON c.id_kota=d.kota_id where a.kode_transaksi='$kode_transaksi'")->row_array();
				$data['record'] = $this->db->query("SELECT a.kode_transaksi, b.*, c.nama_paket,  c.username_paket FROM `tb_paket_penjualan` a JOIN tb_paket_penjualandetail b ON a.id_penjualan=b.id_penjualan JOIN tb_paket c ON b.id_paket=c.id_paket where a.kode_transaksi='" . $kode_transaksi . "'");
				$data['total'] = $this->db->query("SELECT a.kode_transaksi, a.resi,  a.proses,  sum((b.harga_paket*b.jumlah)-(0*b.jumlah)) as total, sum(0*b.jumlah) as total_berat FROM `tb_paket_penjualan` a JOIN tb_paket_penjualandetail b ON a.id_penjualan=b.id_penjualan JOIN tb_paket c ON b.id_paket=c.id_paket where a.kode_transaksi='" . $kode_transaksi . "'")->row_array();
				$this->template->load('home/template', 'home/paket/view_tracking_view', $data);
			} else {
			...
			$this->template->load('home/template', 'home/paket/view_tracking', $data);
		}
	}
}
