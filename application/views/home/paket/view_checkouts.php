...
<?php
        $total = $this->db->query("SELECT sum((a.harga_paket*a.jumlah)-(0*a.jumlah)) as total, sum(0*a.jumlah) as total_berat FROM `tb_paket_penjualantemp` a JOIN tb_paket b ON a.id_paket=b.id_paket where a.session='" . $this->session->idp . "'")->row_array();
                ...
                <div class="card-body">
                    ...
                    <table class="table table-borderless" style="width: 100%">
                        ...
                        foreach ($record->result_array() as $row) {
                            $sub_total = (($row['harga_paket'] - $row['diskon']) * $row['jumlah']);
                            if ($row['diskon'] != '0') {
            ...
            <div class="card w-100 shadow mb-2 bg-white rounded">
                <div class="card-body">
                    ...
                    <table class="table table-borderless">
                        ...
                        <tr>
                            <td>
                                <button type='submit' name='submit' id='oksimpan' class='btn btn-success btn-block btn-sm' >
                                    Bayar
                                </button>
                            </td>
                        </tr>
